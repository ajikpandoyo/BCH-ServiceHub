...
<div x-data="{ searchQuery: '' }">
    <div class="content-wrapper">
        ...
        <div class="section-header">
            <h1>Verifikasi Pengajuan Kunjungan Visit</h1>
        </div>

        <!-- Search and Actions -->
        <div class="content-header">
            <div class="search-wrapper">
                <form method="GET" action="{{ route('admin.verifikasi.kunjungan.index') }}" class="search-box">
                    <input type="text" 
                           class="search-input" 
            ...
            <div class="action-buttons">
                ...
                <x-button.secondary url="{{ route('admin.verifikasi.kunjungan.export', request()->query()) }}" title="Export" icon="fas fa-download"></x-button.secondary>
            </div>
        </div>
            ...
            <table class="custom-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $item->instansi }}</td>
                        <td>
                            <div>{{ \Carbon\Carbon::parse($item->tanggal_kunjungan)->format('d F Y') }}</div>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    <a href="{{ route('admin.verifikasi.kunjungan.show', $item->id) }}" class="dropdown-item">
                                        <i class="fas fa-eye"></i> Detail
                                    </a>
                                    @if($item->status === 'pending')
                                    <form action="{{ route('admin.verifikasi.kunjungan.approve', $item->id) }}" method="POST">
                                        @csrf
                                        @method('POST')
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-4">
                            <div class="empty-state">
                                ...
                                <p>Tidak ada data kunjungan visit</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="pagination-wrapper">
            <div class="pagination-info">
                <span>Showing {{ $kunjungan->firstItem() }}-{{ $kunjungan->lastItem() }} of {{ $kunjungan->total() }}</span>
            </div>
            <!-- {{ $kunjungan->links() }} -->
            <div class="pagination">
                <!-- {{-- Tombol Prev --}} -->
                <button class="page-btn" {{ $kunjungan->onFirstPage() ? 'disabled' : '' }}
                    onclick="window.location='{{ $kunjungan->previousPageUrl() }}'">Prev</button>

                <!-- {{-- Tombol Halaman --}} -->
                @for ($i = 1; $i <= $kunjungan->lastPage(); $i++)
                    @if ($i == $kunjungan->currentPage())
                        <button class="page-btn active">{{ $i }}</button>
                    @elseif ($i == 1 || $i == $kunjungan->lastPage() || abs($i - $kunjungan->currentPage()) <= 1)
                        <button class="page-btn"
                            onclick="window.location='{{ $kunjungan->url($i) }}'">{{ $i }}</button>
                    @elseif ($i == 2 && $kunjungan->currentPage() > 4)
                        <span class="dots">...</span>
                    @elseif ($i == $kunjungan->lastPage() - 1 && $kunjungan->currentPage() < $kunjungan->lastPage() - 3)
                        <span class="dots">...</span>
                    @endif
                @endfor

                <!-- {{-- Tombol Next --}} -->
                <button class="page-btn" {{ $kunjungan->hasMorePages() ? '' : 'disabled' }}
                    onclick="window.location='{{ $kunjungan->nextPageUrl() }}'">Next</button>
            </div>
        </div>
        ...
        @endif
        @if(session('rejected'))
            <x-toast.reject title="Penolakan Berhasil" condition="{{ session('rejected') }}"></x-toast.>
        @endif
...
function openRejectModal(id) {
    ...
    form.action = `/admin/verifikasi/kunjungan/${id}/reject`; // Sesuaikan dengan route
